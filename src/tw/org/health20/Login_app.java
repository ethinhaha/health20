...
public class Login_app extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Toast.makeText(this, sp.getBoolean("auto_login", false)+"",
		// Toast.LENGTH_LONG).show();
		// Toast.makeText(this,sp.getBoolean("auto_login", true)+"",
		// Toast.LENGTH_LONG).show();
		if (sp.getBoolean("auto_login", false)) {
			...
			// Toast.makeText(this,"123"+sp.getString("user_mail",
			// "")+sp.getString("user_pw", ""), Toast.LENGTH_LONG).show();
		...
		login.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				String msg = message
						.stringQuery("http://medhint.nhri.org.tw/hpforum/test.jsp?email="
				...
				if (msg.indexOf("false") == -1) {
					...
					Toast pop = Toast.makeText(Login_app.this, "輸入錯誤請重新輸入",
							Toast.LENGTH_LONG);
					pop.show();
				}
			}
		...
		auto_cb.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton buttonView,
					boolean isChecked) {
				if (auto_cb.isChecked()) {
					Toast.makeText(Login_app.this, "自動登入已選", Toast.LENGTH_SHORT)
							.show();
					sp.edit().putBoolean("auto_login", true).commit();

				} else {

					Toast.makeText(Login_app.this, "自動登入已取消",
